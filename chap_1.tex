...
The results of the Business as Usual scenario (BAU), gives an idea of what the region could look like in 2100, if the same rate of urbanization, agricultural expansion and deforestation remain on par with the rates of 1990-2010. The projected land use at the 2100 horizon, as can be seen in \ref{fig:BAU_compare}, shows that most of the small remaining forest patches among the agricultural matrix have been replaced by agriculture, and that most of the urban centres have grown, leading to deforestation even in bigger patches surrounding urban zones. The two trends in terms of development are the expansion of the cities on the south shore of the Saint Lawrence, and the growth of eastern city centres such as Granby.  these patterns is on par with the results of the profiling of the municipalities described above, and illustrated in figure \ref{fig:maptrans}). 

In the the Reforestation (R) scenario, urban land is allowed to continue to increase following the 1990-2010 trend, but about as much forest is replanted at every time step. This comes to the cost of agricultural land, for which we see a decrease of about $14.7\%$ (in average), in comparison of the stagnation that is observed in the BAU scenario. This loss is necessary to compensate for the increase in forest  and urban land (see \ref{fig:bar_both} for a detailed break down in the form of a bar chart). Under this scenario, we observe similar urbanization patterns, and some forest patches on the south shore of the saint Lawrence cannot not salvaged. However, many smaller forest patches are conserved and some forested areas grow to surround urban areas (see \ref{fig:Ref_compare}).

Changes to the make up of different forest types only differs in one major way between the two scenarios. The amount of deciduous forest of medium age increases by $24\%$, in comparison to the decrease in $16.9\%$ in the BAU scenario. This is to be expected from the model parameterization, as it is the only (and unchanging) forest type that is planted in the region under the Reforestation scenario (see figure \ref{fig:bar_both}). \\

\subsection{Connectivity Modelling}

\subsubsection*{Historic run (1990 - 2010)}

While our land use change model only managed to approximate the spatial patterns in land use change, it better managed to replicate the observed change in mean current flow in the region. Figure \ref{fig:flow_historic} illustrate that the model results are close to the observed changes. Whereas we observe a drop in about $1.5$ to $3$ \% in mean flow (depending on the species), the model tends to overestimate the drop, and the difference in estimation range from $0.5$ to $1$ points. However, it is clear that given the distribution of flow values in the region, the mean is not capturing all of the nuances in the model predictions. As it can be seen in the histograms of the flow values in figure \ref{fig:hist_historic}, which compares the results of the historic run with the observed flow values, the distribution of flow values is multimodal. We can see that the model managed to approximate pretty well the flow values for most of our species, except for the Wood frog (note that this figure is based on one of the iteration only, but all iterations show a similar pattern). \\
...
\subparagraph*{\textit{Flow distributions}} Due to the multimodal distributions of flow values, it is important to go beyond looking at mean values and visualize  the effect on the histograms of flow values (see figure \ref{fig:hist_1}). We can see that species usually have a trimodal distribution of high, medium and low flow values (American marten displays a more compact distribution with lower variance). Each of the two scenario has a different effects on the distribution of those values

Under the BAU scenarios, it can be see that high flow values are being eroded into low flow values (and to a lesser extent, medium flow values), signifying the replacement of very high quality pixels for connectivity into very low quality pixels. We can therefore label the effects of the BAU scenarios of "directional", in reference to the shift observed in the distribution of a trait in directional selection \citep{haldane_measurement_1954}. On the other hand, in the reforestation scenario, high quality pixels are not being eroded (or are being matched by reforestation), but pixels of medium quality are being replaced by pixels of low quality. We can therefore label the effects of this scenario as "disruptive", in reference to the shift observed in a trait under disruptive selection \citep{haldane_measurement_1954}. Finally, we can mention that climate does not seem to impact changes to the distribution of values noticeably.\\ 

\vspace{1em}

\subparagraph*{\textit{Feature detection}} The results of the feature detection analysis seems to carry similar trends as what is observed for the changes in flow values. As in the mean flow analysis, we see a continual decrease in the number of features detected across all species and scenarios, excepted for the Black Bear for which a small initial decrease seems to stabilize around $10\%$ under the BAU scenario (see \ref{fig:surf_linear_1} and \ref{fig:surf_radar_1}). An opposite pattern to that of flow values is displayed: a much stronger decrease in the number of features is observed under the reforestation scenario than under the BAU scenario.

The impact of climate varies depending on the species considered. For the Black bear and the Wood frog, we see no noticeable differences between the different climate scenarios. However, the responses of the Short-tailed shrew, the American Marten and the Red-back salamander to climate change display apparent differences. The pattern that emerges is that both the baseline and RCP 8.5 scenarios lead to a more important decrease in the number of features detected, with little noticeable differences between them, except for the American Marten.

Finally, it is interesting to remark that, similarly to for the mean  current flow, the width of difference between the two extreme scenarios is smaller for the marten than for the other species, possibly indicating more uncertainty with regards to the real future trajectory of those species to the 2100 horizon.\\ 
