...
The paucity of land use change simulations, in comparison to the slew of established global climate change models available, is mentioned by \cite{costanza_landscape_2019} as a contributing factor. This paucity, we argue, can be explained by the lack of long-term land use data, but also by the opacity that surrounds land use change modelling practices. Although the former is becoming less and less of an issue with the advent of easily accessible remotely sensed data, the latter is a real obstacle to integrating land use change into connectivity models \citep{moulds_open_2015}. Here, we show a flexible framework with transparent parameterization using open-source software, which can be used to simulate landscape change in conjunction with climate change to assess connectivity or any other relevant landscape-level metric. This effort for transparency come with a concern for taking into account uncertainty in land use change simulations, another obstacle to integration mentioned by \cite{costanza_landscape_2019}. By using MCMC (Markov Chain Monte Carlo) based simulations, we can propagate uncertainty surrounding both climate and land use change models. This is crucial, because another critique against the lack of integration is that designing conservation corridors without taking into account future land use and climate change could lead to a false sense of certainty in the efficiency of corridor designs, a mistake that could be reproduced if the uncertainty of integrated models is not better taken into account. Finally, land use change and CC are seen by the research community as impacting connectivity at different scales, with climate change models often being use in larger-scale connectivity studies than land use change. For instance, corridors designed to be adaptive to climate change often span very long distances \citep{mcguire_achieving_2016}. Yet our results show that even noticeable differences between climate scenarios can be observed, even at small scales (i.e. below the ecoregion level).

% then go on to talk about climate chnage scale, this will end part 1

% Review results and contrast with other papers integratiing LUC and CC models
Other studies have attempted to go beyond those issues and managed to integrate climate and land use change models to some degree. For example, \cite{albert_applying_2017}, on which this work is based, considered which habitat patches network would be most resilient to future change using multi scale metrics in Monteregie. \cite{leonard_landscape-scale_2017} used systematic conservation planning methods to identified core area and linkages under an energy development land use change scenario in the northeastern US. In another study, \cite{dilts_multiscale_2016} use a range of graph theory-based metrics to forecast the changes to diverse aspect of habitat connectivity for the Mohave ground squirrel under multiple climate and land use change scenarios. Although our results, contrary to those studies, do not allow us to derive a tangible predictions for our study species such as a plan for a protected area network, we still provide valuable insight for the design of such hypothetical network. 

For instance, by looking at a simple metric (mean flow), we have shown that some species will react more simply and linearly than others. We saw a simple and linear response to change for species which, in our model, had dependency on forest types that were expected to be impacted only by land use change, such as the Redbacked salamander and the Wood frog. This was not the case for species which have are dependant on coniferous forests or mixed type forests, more at risks from climate change, and therefore more likely to suffer from the interaction of the two drivers. The results of our simple reforestation scenario highlights that it might be hard to plan for species with different needs. Restoration actions remain very uncertain: the impact of reforesting will depend on what is replanted and where it is replanted. Forest management could turn out to be critical for species dependant on forest types more vulnerable to a changing climate.

Our implementation of the feature detection algorithm opens the door to more research on how future land use and climate change will impact the complexity of the species' flow.

% Limitations
This work constitute progress over past project on which this work is built. We covered a larger extent, improved on the land use change model from \cite{albert_applying_2017}, and demonstrated the flexibility of our framework for supporting alternative scenarios. But besides the important methodological contribution of this chapter, it is not without limitations. \\
% Could argue this kicks down uncertainty to parameters
% Ccl: We can derive some clues as to how our species will react to climate change and land use change, and as to how best to plan for the conservation of their connectivity needs. 
